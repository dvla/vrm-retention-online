...
//    }
//
//    "redirect to Before You Start page when back button is pressed and dealer details is not in cache" in new WithApplication {
//      val request = FakeRequest().withFormUrlEncodedBody()
//      val result = vehicleLookupResponseGenerator().back(request)
//
...
//    }
//
//    "redirect to Before You Start when back button clicked and there are no trader details stored in cache" in new WithApplication {
