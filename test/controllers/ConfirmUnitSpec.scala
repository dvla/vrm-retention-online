...
import views.vrm_retention.Confirm.{StoreBusinessDetailsCacheKey, KeeperEmailCacheKey, StoreDetailsConsentId, KeeperEmailId}
  ...
  "submit" should {
    ...
    "redirect to Payment page when valid submit and user type is Business" in {
      val request = buildRequest().
        withCookies(CookieFactoryForUnitSpecs.vehicleAndKeeperLookupFormModel(keeperConsent = UserType_Business)).
        withCookies(CookieFactoryForUnitSpecs.vehicleAndKeeperDetailsModel()).
        withCookies(CookieFactoryForUnitSpecs.businessDetailsModel())
    ...
    "redirect to Payment page when valid submit and user type is Keeper" in {
      val request = buildRequest().
        withCookies(CookieFactoryForUnitSpecs.vehicleAndKeeperLookupFormModel(keeperConsent = UserType_Keeper)).
        withCookies(CookieFactoryForUnitSpecs.vehicleAndKeeperDetailsModel()).
        withCookies(CookieFactoryForUnitSpecs.businessDetailsModel())
      val result = confirm.submit(request)
      whenReady(result) { r =>
