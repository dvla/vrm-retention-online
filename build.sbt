...
lazy val vrmRetentionEligibilityProject = vrmRetentionEligibility("0.11-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
lazy val vrmRetentionRetainProject = vrmRetentionRetain("0.10-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
//lazy val auditProject = audit("0.3").disablePlugins(PlayScala, SbtWeb) // Disabled for now due to it needing to be in scala 2.11 but the webapp is still scala 2.10.
